...
class DOTA:
    def __init__(self, basepath, confidence = False):
...
#         self.parseMode = parseMode # 
    ...
    def createIndex(self):
        for filename in self.imgpaths:
            if self.confidence == False:
                ...
                objects = util.parse_dota_poly_confidence(filename)
                imgid = util.custombasename(filename)
                self.ImgToAnns[imgid] = objects
                for obj in objects:
                    cat = obj['name']
    ...
    def loadImgs(self, imgids=[]):
        ...
        for imgid in imgids:
            try:
                filename = os.path.join(self.imagepath, imgid + '.npy')
                print('filename:', filename)
                img = np.load(filename)
                imgs.append(img)
            except:
                filename = os.path.join(self.imagepath, imgid + '.tiff')
                print('filename:', filename)
