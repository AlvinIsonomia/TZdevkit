"""
    some basic functions which are useful for process TZ data
"""
import sys
import codecs
import numpy as np
...
def parse_dota_poly_confidence(filename):
    """
        AFTER INFERENCE
        parse the dota ground truth in the format:
        [(x1, y1), (x2, y2), (x3, y3), (x4, y4)]
    ...
    for line in lines:
        ...
        object_struct['poly'] = [(float(splitlines[2]), float(splitlines[3])),
                                 (float(splitlines[4]), float(splitlines[5])),
                                 (float(splitlines[6]), float(splitlines[7])),
                                 (float(splitlines[8]), float(splitlines[9]))
                                 ]
        gtpoly = shgeo.Polygon(object_struct['poly'])
        object_struct['area'] = gtpoly.area
        objects.append(object_struct)
...
#                 if filename not in filedict:
            ...
            # filedict[filename].write(' '.join(poly) + ' ' + idname + '_' + str(round(float(confidence), 2)) + '\n')
                ...
                filedict[filename].write(idname + ' ' + confidence +' '+' '.join(poly)  + '\n')
