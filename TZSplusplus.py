...
import numpy as np
import math
from TZ_utils import GetFileFromThisRootDir
import cv2
...
import time
# import tifffile
from DataProcessing import get_shape, npread_plus 
...
class splitbase():
    def __init__(self,
        ...
        # FIXME : 泛用性
        self.shape_dict = get_shape(shape_file)
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        img = npread_plus(os.path.join(self.imagepath, img + self.ext), input_dict = self.shape_dict)
        ...
        if(h != self.subsize or w != self.subsize):
            subimg = np.pad(subimg,((0,self.subsize-w),(0,self.subsize-h),(0,0))) 
            np.save(outdir,subimg)
        else:
            np.save(outdir,subimg)
    ...
    def savepatches(self, resizeimg, objects, subimgname, left, up, right, down):
        ...
        # img = npread_plus(os.path.join(self.imagepath, name + self.ext), input_dict = self.shape_dict)
        # mask_poly = []
    ...
    def SplitSingle(self, name, rate, extent):
        ...
        weight = self.shape_dict[name][1]
        height = self.shape_dict[name][0]
        ...
        while (left < weight):
            ...
            while (up < height):
                ...
                self.savepatches(name, objects, subimgname, left, up, right, down)
                if (up + self.subsize >= height):
                    break
                else:
                    up = up + self.slide
...
if __name__ == '__main__':
    ...
    # split.splitdata(0.4)
    #
    input_dir = '//data/datasets/DOTA/TZ/'
    output_dir = os.path.join(input_dir,'images')
    split_dir = '//data/datasets/DOTA/TZ/split'
    shape_file = '//data/datasets/DOTA/TZ/shape_file.txt'
       
    
    # for npy in npys:
    start = time.time()
    split = splitbase(input_dir,
                       split_dir,
                       shape_file,
                      gap=100,
                      subsize=1024,
