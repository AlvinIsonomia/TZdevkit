...
class splitbase():
    def __init__(self,
                 ...
                 outpath,
                 code = 'utf-8',
                 gap=100, # overlap between two patches
                 subsize=1024,
                 thresh=0.7, # 若裁剪时instance惨遭腰斩，是否保存
                 choosebestpoint=True, # 确定点的顺序，整挺好
                 ext = '.png' #文件格式
                 ):
        """
    ...
    def saveimagepatches(self, img, subimgname, left, up):
        '''
        save as .npy files with numpy
        '''
        subimg = copy.deepcopy(img[up: (up + self.subsize), left: (left + self.subsize)])
        np.save(os.path.join(self.outimagepath, subimgname + '.npy'),subimg)
    ...
    def savepatches(self, resizeimg, objects, subimgname, left, up, right, down):
        '''
        调用被魔改的 saveimagepatches 就可以保存.npy的图片了
        '''
